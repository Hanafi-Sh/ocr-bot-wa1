...
// =========================
    ...
    client.on('message', async msg => {
        ...
        if (perintahKecil === '!ram') {
            ...
            try {
                const memoriTerpakaiBytes = execSync('cat /sys/fs/cgroup/memory.current').toString().trim();
                const batasMemoriBytes = execSync('cat /sys/fs/cgroup/memory.max').toString().trim();
                const memoriTerpakaiMB = (parseInt(memoriTerpakaiBytes) / 1024 / 1024).toFixed(2);
                const batasMemoriMB = batasMemoriBytes === 'max' ? 'Maksimal Server' : (parseInt(batasMemoriBytes) / 1024 / 1024).toFixed(2) + ' MB';
                const diskSpace = execSync("df -h /home/runner/workspace | awk 'NR==2 {print $3 \" / \" $2}'").toString().trim();
